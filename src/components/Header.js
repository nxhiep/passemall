		...
		<header>
			<Container style={{ display: "flex" }}>
				<div className="header-app" onClick={() => router.push("/")}>
					<img src="/images/logo-landing.png" alt="logo-app" height="50px"></img>
				</div>
				{isMobile ? null : (
					<div style={isMobile ? { dispaly: "none" } : { marginLeft: "auto", display: "flex", alignItems: "center" }}>
						<Link href={getLink()}><a style={{ fontSize: "18px" }}>{studying ? "Learn" : "Home"}</a></Link>
						<Link href={getLink("test")}><a style={{ fontSize: "18px" }}>Test</a></Link>
						<Link href={getLink("review")}><a style={{ fontSize: "18px" }}>Review</a></Link>
					</div>
				)}
