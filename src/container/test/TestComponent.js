...
const CountDownUI = ({ timeLeftReducer, id, setTimeLeftState, onContinue, endTest, level }) => {
    ...
    return (
        <div style={{ background: "#3f51b5", height: "40px", width: "140px", marginLeft: "auto", color: "#fff", textAlign: "center" }}>
            {convertTime(timeLeftReducer.data[id] ? timeLeftReducer.data[id].timeLeft : -1)}
        </div>
    )
}
const TestQuestionPanelUI = ({ endTest = () => { }, initial = 0, questionProgress = {}, className = "", loadGame = () => { }, gameState, testInfoId, appId, timeTest, passPercent, questionIds, index = 0, onBookmark, appInfoState, onContinue = () => { } }) => {
...
const TestProgressPanelUI = ({ gameState, appInfo, onBookmark, setShowLeftPanel }) => {
    ...
    return (
        <div style={{ display: "flex", marginBottom: "40px", alignItems: "center", flexDirection: isMobile ? "column" : "row" }}>
            ...
            <Grid container spacing={1} direction={isMobile ? "column" : "row"} className="list-level">
                <Grid item xs={isMobile ? 12 : 4} onClick={() => handleClickLoadGame(Config.EASY_LEVEL, level)} style={{ display: "flex", background: level === Config.EASY_LEVEL ? "#fff" : "#F0F0F3" }} className="button-level">
                ...
                <Grid item xs={isMobile ? 12 : 4} onClick={() => handleClickLoadGame(Config.HARD_LEVEL, level)} style={{ display: "flex", background: level === Config.HARD_LEVEL ? "#fff" : "#F0F0F3" }} className="button-level">
                ...
                <Grid item xs={isMobile ? 12 : 4} onClick={() => handleClickLoadGame(Config.VERY_HARD_LEVEL, level)} style={{ display: "flex", background: level === Config.VERY_HARD_LEVEL ? "#fff" : "#F0F0F3" }} className="button-level">
                    <div style={{ display: "flex", flexDirection: "column" }}>
