...
import { ArrowBack as ArrowBackIcon, Assignment as AssignmentIcon, Check as CheckIcon, Close as CloseIcon, Done as DoneIcon, ArrowRightAlt as ArrowRightAltIcon, Lock as LockIcon, LockOpen as UnLockIcon, ViewList as ViewListIcon, DoneAll as DoneAllIcon } from '@material-ui/icons';
...
class StudyViewScreenUI extends Component {
    ...
    constructor(props) {
        ...
        this.props.getTopicById(this.state.id);
    }
        ...
        if (!this.state.isMobile) {
            return (
                <div className="body-panel">
                    <Header isStudy={true} studying={true} />
                        ...
                        >
                            ...
                            <Grid item xs={12} sm={12} md={7} lg={8} className="right-panel">
                                ...
                                >
                                    ...
                                    >
                                        Next Part <ArrowRightAltIcon />
                                    </Button>
                                </Grid>
                            </Grid>
                        </Grid>
                    </Container>
                    <FloatingButtonTest currentTopic={this.state.currentTopic} />
        ...
        if (this.state.isMobile) {
            return (
                <div className="body-panel">
                    ...
                    {(this.state.count % 2 === 0 && this.state.count !== 0) ? <DialogForMobile handleInstalled={() => this.setState({ count: -1 })}></DialogForMobile> : null}
                    <ShowImage />
                </div>
            )
        }
        ...
        return (
            <div>
                <div style={congratulationTopic ? { display: 'none' } : {}}>
                    {isMobile ? null : <QuestionProgressPanel topic={currentTopic} />}
                    {
                        currentTopic ?
                            <QuestionsPanelTS
                                appInfo={appInfo}
    ...
    return (
        <Grid container alignItems="center" justify="space-between" className="congratulation-alert-panel">
            ...
            <div className="title">
                ...
                <span>You must complete previous topic!</span>
            </div>
...
const QuestionProgressPanelUI = (props) => {
    ...
    let listQuestion = gameState.questions;
...
const QuestionItemProgress = ({ index, question, currentQuestion }) => {
    ...
    if (question.progress.progress[progressLength - 1] === 1) {
        statusStr = ' correct';
        icon = <DoneAllIcon className="icon" />;
    } else if (question.progress.progress[progressLength - 1] === 0) {
        statusStr = ' incorrect';
...
const TopicInfoPanelUI = ({ topicId = -1, topicState, appInfo }) => {
    let isMobile = isMobileFunctions();
    ...
    if (isMobile) {
        return (
            <Grid container alignItems="center" style={{ display: "inline-block" }}>
                <IconButton onClick={() => {
                    router.push("/" + appInfo.appNameId);
                }}><ArrowBackIcon /></IconButton>
                <span style={{ textAlign: "left" }}>{topic.name}</span>
            </Grid>
        );
    }
    return (
...
const TopicTreePanelUI = ({ topicState, parentId, currentQuestionId, onChangeTopic, resetTopicProgress, openAlert, gameState }) => {
