...
import { loadGame, removeLastGame, resumeGame, startNewGame, updateListGame, resetQuestionProgress, } from '../actions/game';
...
function* startGame() {
    ...
    let topic = topicReducer.data[topicId]
    let appId = action.appId;
    let gameType = action.gameType;
    let testSetting;
    if (action.setting) {
            ...
            if (gameReducer.gameType != Config.TEST_GAME) {
                ...
                if (currentTopic && gameReducer.gameType == Config.STUDY_GAME) {
                    ...
                    if (currentParentTopic) {
                        let sumProgress = 0;
                        let count = 0;
                        topicState.list.forEach(topic => {
                            if (topic.parentId === currentParentTopic.id) {
                                count++;
                                sumProgress += topic.getPercentComplete();
                            }
                        });
                        yield put(calcularParentTopicsProgress(currentParentTopic, Math.round(sumProgress / count)));
                    }
                }
            }
        }
